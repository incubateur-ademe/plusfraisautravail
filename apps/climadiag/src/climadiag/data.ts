// Port of plusfraichemaville-site src/components/climadiag/climadia-indicateurs-data.tsx
import type { ClimadiagTypeJour, ClimadiagYear } from './types';

type LegendByYear = Record<ClimadiagYear, string[]>;

const jourTresChaudLegend = (seuil: number): LegendByYear => ({
  2030: [
...
const nuitsChaudesLegend = (seuil: number): LegendByYear => ({
  2030: [
...
const joursVdcLegend: LegendByYear = {
  2030: [vdcDefinition, 'L’augmentation du nombre de jours en vagues de chaleur est déjà perceptible et se poursuivra sur l’ensemble du pays d’ici l’horizon 2030.'],
  2050: [vdcDefinition, 'L’augmentation du nombre de jours en vagues de chaleur est déjà perceptible et se poursuivra sur l’ensemble du pays d’ici l’horizon 2050.'],
  2100: [vdcDefinition, 'L’augmentation du nombre de jours en vagues de chaleur est déjà perceptible et se poursuivra sur l’ensemble du pays d’ici l’horizon 2100.'],
};

export const climadiagIndicateursData: Record<
  ClimadiagTypeJour,
  { title: string; picto: string; legend: (seuil: number) => LegendByYear }
> = {
  jours_chauds: {
    title: 'Nombre annuel de jours très chauds',
    picto: 'climadiag-jour-tres-chaud',
    legend: jourTresChaudLegend,
  },
  nuits_chaudes: {
    title: 'Nombre annuel de nuits chaudes',
    picto: 'climadiag-nuits-chaudes',
    legend: nuitsChaudesLegend,
  },
  jours_vdc: {
    title: 'Nombre annuel de jours en vague de chaleur',
    picto: 'climadiag-jour-vdc',
    legend: () => joursVdcLegend,
  },
};

export const climadiagColorLegend = [
  { label: 'Période de référence (1976 - 2005)', color: 'cd-dot-ref' },
  { label: 'Valeur basse des projections climatiques autour de', color: 'cd-dot-basse' },
  { label: 'Valeur médiane des projections climatiques autour de', color: 'cd-dot-mediane' },
  { label: 'Valeur haute des projections climatiques autour de', color: 'cd-dot-haute' },
];

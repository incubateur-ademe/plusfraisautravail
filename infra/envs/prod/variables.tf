...
variable "cms_min_scale" {
  type        = number
  description = "Minimum number of always-warm cms container instances. Temporarily forced to 1 while debugging the container never reaching ready - rules out the scale-from-zero cold-start path."
  default     = 1
}
